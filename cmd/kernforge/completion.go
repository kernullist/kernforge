...
var slashCommandDescriptions = map[string]string{
	...
	"gate":                    "Show runtime gate status, or clear/restore this session's gate review.",
	...
	"decision":                "Open the local cross-project decision journal and preference dashboard.",
...
var slashSubcommandDescriptions = map[string]map[string]string{
	...
	"gate": {
		...
		"clear":             "Dismiss this session's review from the runtime gate (keeps review files; default session scope).",
		"clear --session":   "Dismiss the gate for this session only (default).",
		"clear --workspace": "Also write a workspace dismissal file (optional).",
		"clear --reason":    "Attach a short operator reason to the dismissal.",
		"restore":           "Re-attach the dismissed review to this session's runtime gate.",
	},
	"review": {
	...
	"goal": {
		"--file":          "Load a goal objective from a markdown file, then design and run.",
		"@GOAL.md":        "Load a goal objective from a markdown file, then design and run.",
		"--max-iterations": "Cap autonomous loop iterations.",
		"--time-budget":   "Wall-clock budget for the goal run.",
		"--token-budget":  "Token budget estimate for the goal run.",
		"--criteria":      "User acceptance criteria checklist items.",
		"--research":      "Enable research_mode (bounded by default).",
		"--worktree":      "Isolate the goal in a git worktree.",
		"--require-review": "Require independent semantic review for completion.",
	},
	"model": {
...
func (rt *runtimeState) slashArgumentSuggestions(commandName string, fields []string, endsWithSpace bool) ([]string, int, bool) {
	firstLevel := map[string][]string{
		...
		"finish":              {"--disclose"},
		"goal":                {"--file GOAL.md", "@GOAL.md", "--max-iterations", "--criteria", "--research", "--worktree", "--require-review"},
	...
	case "gate":
		...
		if strings.EqualFold(fields[0], "clear") {
			if len(fields) == 2 {
				return []string{"--session", "--workspace", "--reason"}, 1, true
			}
			return nil, 0, false
		}
		return nil, 0, false
