...
func (t CreateGoalTool) Definition() ToolDefinition {
	return ToolDefinition{
		...
		Description: "Create an internal active thread goal only when the user or system/developer instructions explicitly ask to start, activate, or use built-in goal tracking.\n" +
			...
			"For the user-facing Kernforge command flow, tell the user to use /goal <objective> (designs a plan then runs until complete), /goal @GOAL.md, or bare /goal to resume an incomplete goal. Progress prints during the loop; Esc interrupts.\n" +
