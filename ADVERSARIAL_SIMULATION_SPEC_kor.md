...
을 바탕으로, 공격자 관점의 해석을 빌린 lightweight risk lens로서 "현재 변경이나 현재 상태에 어떤 운영상 취약 지점이나 관찰 공백이 남아 있는가"를 구조적으로 점검하는 기능이다.
...
즉 핵심은 exploitability를 증명하는 것이 아니라
"어디가 먼저 흔들릴 가능성이 있는가"를 reviewable evidence로 만드는 것이다.

중요한 범위 제한:

1. 현재 simulation은 규칙 기반 heuristic evaluation이다.
2. 실제 exploit simulation, bypass validation, stealth 검증을 수행하지 않는다.
3. profile 이름은 공격자 관점의 해석 틀을 나타낼 뿐, 실제 공격 수행 capability를 뜻하지 않는다.
...
3. `Adversarial Simulation Profiles`가 그 결과를 입력으로 읽어 공격자 관점의 risk 해석을 수행
...
/simulate forensic-blind-spot provider-visibility
```
...
1. 현재 변경/상태에서 tamper pressure가 먼저 걸릴 수 있는 부분은 어디인가?
2. integrity / signing / registration / configuration이 상대적으로 약하게 보이는 지점이 있는가?
...
2. `provider-registration-risk-surface`
3. `repeated-failure-path-keeps-tamper-risk-open`

### 7.2 stealth-surface

질문:

1. 조용한 실패나 관찰 공백이 생기기 쉬운 곳은 어디인가?
...
Simulation 결과는 단순 텍스트가 아니라 evidence/hook에 다시 쓰기 쉬운 heuristic output이어야 한다.
...
1. `/investigate start driver-visibility <target>`
2. `/verify`
...
즉 초기에는 strong enforcement보다 advisory heuristic이 맞다.
