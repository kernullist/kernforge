...
- `RequestEnvelope`, `ToolContract`, `TurnRuntimeState`, final gate decision, scenario replay fixture, 선택적 `request_runtime` migration mode를 포함하는 request handling v2 runtime. review-only/plan-only turn은 read-only로 유지하고, explicit edit은 tool-driven으로 유지하며, compaction/history rewrite 뒤에도 mutable verification context를 보존한다. git mutation은 사용자가 명시적으로 요청한 경우에만 허용하고, shadow divergence log는 민감한 prompt/provider transcript 없이 `.kernforge/request_runtime_shadow` 아래에만 저장한다.
...
- 도구는 결과를 정직하게 보고하고 fail-closed로 동작한다. 앞 단계가 실패한 셸 명령 체인을 더 이상 성공으로 보고하지 않고 실제 exit code를 보존하며, `max_results`로 잘린 `grep` 결과에는 truncation 안내가 붙고, `apply_patch`는 기존 파일을 `Add File`로 덮어쓰지 않고 삽입 위치와 trailing newline 상태를 보존하며, 배열 형태 셸 인자로 action boundary를 우회할 수 없고, `git_diff`/`git_status`가 CRLF 경고를 본문에 섞지 않으며, MCP 네임스페이스 도구 이름이 충돌하지 않아 서로 다른 서버의 도구가 조용히 누락되거나 mis-route되지 않는다.
- request 분류는 read-only 기본 안전 규칙을 따른다. 모호한 요청은 파일/git 변경 권한을 얻는 대신 read-only/최소 권한 쪽으로 떨어진다. run/build/test 전용 요청은 명령 실행으로 분류되고, update/patch/change 같은 단어가 포함된 설명형 질문은 read-only로 유지되며(verb 매칭이 단어 경계를 보므로 prefix/suffix/multistage 같은 무관한 단어를 명령으로 오인하지 않는다), git 부정/질문("커밋하지 마", "git push가 뭐야?")은 git 권한을 얻지 못하고, 이전의 오래된 편집이 이후 read-only 후속 요청에 변경 권한을 물려주지 않는다. 코드를 분석해서 문서로 정리해 달라는 요청은 코드 수정 lifecycle이 아니라 문서 작성으로 분류된다.
...
- `/create-driver-poc <driver-name> [--type objectfilter|minifilter|registryfilter|wfpcallout]`으로 x64 전용 C++20 MSVC/WDK POC 드라이버 솔루션을 생성한다. `--type`을 생략하면 기존 SCM 설치/시작, device open, IOCTL ping, unload/delete 기본 POC를 생성한다. typed template은 object manager 프로세스/쓰레드 핸들 access stripping, filesystem minifilter open/rename/delete 유저 모드 판단 경로, registry create/open/set/delete/rename callback 차단, WFP outbound callout 차단 계약을 생성한다. 완료 시 security workflow handoff와 `.kernforge/security/workflow_seed.json`을 남긴다.
- 고위험 finding은 위험도 점수표, 먼저 볼 소스 위치, 시작 파일에서 거기까지의 파일 확장 경로, 대표 루트에서 이어지는 호출 경로를 함께 보여준다.
- `compile_commands.json`이나 build context가 있으면 후속 네이티브 fuzzing 품질이 올라가고, 없으면 왜 막히는지 먼저 설명한 뒤 진행 여부를 묻는다. build-only 실패는 실패 클래스 분류와 1–3회 harness self-repair, durable blocker 기록을 거친다.
- 결과 산출물은 `.kernforge/fuzz/<run-id>/` 아래의 `report.md`, `harness.cpp`, `plan.json` 등에 저장된다. IOCTL surface면 `ioctl_contract.json`과 multi-call sequence seed도 함께 남긴다.
- native crash는 feasibility gate(`target_plausible` / `spurious`)를 거친다. harness-only stack은 validated finding으로 승격하지 않는다.
...
- 잘못된 patch는 BOM, CRLF/CR, 주변 설명문, code fence, quoted path 같은 흔한 wrapper 문제를 먼저 정규화한다. 같은 invalid patch signature가 반복되면 동일 patch 재시도 대신 target file을 다시 읽고 새 patch를 만들도록 유도한다. hunk 매칭은 대상 파일 선두의 UTF-8 BOM(read 출력에서는 보이지 않음)도 허용하고 write 시 BOM을 보존하므로, BOM 파일의 line-1 hunk가 영원히 mismatch되는 문제가 없다.
- edit target mismatch나 pre-write review 차단처럼 stale-context 복구 신호가 생긴 뒤에는 넓은 recovery patch를 workspace에 적용하지 않고 연기한다. 차단된 pre-write proposal은 적용된 상태가 아니므로, 다음 edit tool은 `read_file`, `grep`, `git_diff`로 현재 파일 또는 현재 diff를 다시 확인한 뒤에만 실행된다. `replace_in_file` mismatch는 expected/current context 진단을 포함하며, 모호 매칭은 bare "not found" 대신 ambiguous로 구분한다. 매 mismatch마다 reanchor가 다시 필요하고, 문서 산출 턴에서는 mismatch 이후 `write_file` 성공 전까지 context edit 도구를 다시 열지 않는다. 모델은 거절된 proposal의 delta가 아니라 현재 상태에 바로 적용 가능한 좁은 standalone patch를 다시 제출해야 한다. 턴의 mismatch 예산이 하나 남으면 파일 전체 재작성용으로 `write_file`(최종 전체 내용 제출)로 전환하도록 유도하고, 편집이 한 번 성공하면 턴별 mismatch 예산이 리셋되며, mismatch 중단 보고는 "아무것도 적용 안 됨"이라고 뭉뚱그리는 대신 이번 턴에서 이미 적용된 편집 목록을 정확히 표시한다.
...
- 사용자 출력은 plain-language이며 locale을 따른다. 내부 코드네임(request class, lifecycle kind, route mode, `needs_review` 같은 gate status, intervention 이름)은 사람이 읽는 텍스트로 렌더링하고 raw 값은 machine/진단 field에만 남긴다. "approved with warnings: 0 warning(s)" 같은 모순 요약은 실제 사유로 바뀌고, 저장된 review 보고서는 결과(요약, 변경 파일, finding, 다음 명령)를 먼저 보여준 뒤 lifecycle/ledger 진단을 끝의 진단 섹션으로 옮기며, MCP review 응답은 JSON 앞에 사람용 요약을 먼저 둔다. 노이즈도 정리했다. false-positive secret redaction이 멀쩡한 문자열 리터럴을 훼손하지 않고, PR 보고서가 Git CRLF 경고를 제거하며, 라이브 상태 footer가 단어 중간에서 잘리지 않는다.
...
- 독립 reviewer가 전혀 없는 single-model route에서는 암시적 model-backed review를 `skipped_single_model_route`로 자동 생략하고 `Run model review now?` consent prompt를 띄우지 않는다. 메인 모델이 자기 수정안을 리뷰하는 것은 독립 신호가 아니므로 deterministic check와 diff preview가 gate가 된다. 명시적 `/review`는 그대로 동작하고, `review.model_review_consent = always`가 암시적 self-review를 계속 실행하는 opt-in이며, self-review가 실행되는 경우에도 그 finding은 advisory(경고 표기, 쓰기 차단 없음)로만 반영된다.
...
여기서 `driver-visibility` preset은 드라이버 로드 root cause를 깊게 파고드는 분석기가 아니라, 현재 시점의 드라이버 가시성, verifier 상태, 관련 artifact 존재 여부를 빠르게 남기는 lightweight triage snapshot이다. platform/HVCI readiness가 필요하면 `/investigate start platform-security`를 함께 쓴다.
...
| `-permission-mode <mode>` | 권한 모드 지정: `plan`, `edit`, `full` (기본 `plan`); 레거시 이름과 Codex 프로파일 id도 수용 |
| `-y` | 모든 권한 자동 승인 (`full`) |
...
Kernforge는 시작 시 사용자 skill 디렉터리에 `$goal-to-slice-planner` 생산성 스킬을 함께 배포합니다. 기본 활성화되지는 않으며, 큰 작업을 구현 전에 작은 검증 가능한 slice로 나누고 싶을 때 명시적으로 호출합니다.

```text
$goal-to-slice-planner 이 기능을 검토 가능한 구현 slice와 검증/문서 작업으로 나눠줘
```
...
#### 원격(streamable_http) MCP 서버 연결과 문제 해결

원격 MCP 서버는 다음처럼 등록하면 즉시 연결을 시도합니다.

```text
/mcp add <name> --transport http <url> --header "Authorization: Bearer <token>"
```

`/mcp`로 상태를 확인하세요. 연결되면 `tools=N resources=N prompts=N`, `auth=bearer_token`, `url=...`이 표시되고 도구가 `mcp__<name>__*`로 노출됩니다. 실패하면 `error=` 항목에 원인이 나옵니다.

**`context deadline exceeded`** (가장 흔함): MCP 프로토콜이 아니라 네트워크 도달 자체가 안 되는 신호입니다(TCP/HTTP 응답 타임아웃). 다음을 순서대로 확인하세요.

1. **서버 listen 확인** — 서버 호스트에서 `netstat -ano | findstr :<port>`가 `0.0.0.0:<port>` 또는 실제 IP를 보여야 합니다. `127.0.0.1`만이면 루프백 전용이라 외부에서 붙을 수 없습니다(네트워크 인터페이스에 bind해 재시작).
2. **클라이언트→서버 TCP 도달성** — kernforge 호스트에서 `Test-NetConnection <server-ip> -Port <port>`. `TcpTestSucceeded: False`면 방화벽/라우팅 문제입니다(ICMP ping이 막혀 있어도 무관 — TCP만 봅니다).
3. **HTTP 레벨 확인** — `initialize`를 직접 던져 `HTTP 200`과 `serverInfo`가 오는지 봅니다.
...
VM 랩(VMware NAT 등)에서 가장 흔한 원인은 **게스트 OS의 인바운드 방화벽**입니다. 호스트와 게스트가 같은 서브넷이어도(예: 호스트 VMnet8 `192.168.x.1`, 게스트 `192.168.x.N`) 게스트 방화벽이 포트를 막으면 ARP(L2)는 응답해도 TCP는 실패합니다. 게스트에서 **클라이언트 IP만 허용**으로 여세요.
...
그 밖의 `error=` 패턴: **`401`/인증 오류**는 bearer 토큰 불일치(`--header` 값 확인), **`404`**는 URL 경로 오류(예: `/mcp` 누락)입니다.

네트워크/방화벽을 고친 뒤에는 재등록 없이 `/reload` 또는 `/mcp disable <name>` → `/mcp enable <name>`로 재연결됩니다.

도구가 수십 개인 서버는 설정 항목에 `"defer_tool_schemas": true`를 넣으면 도구 목록만 먼저 노출하고 InputSchema는 호출 시 가져와 컨텍스트를 아낍니다.

**보안**: bearer 토큰은 `~/.kernforge/config.json`에 평문으로 저장됩니다. 커널 R/W나 write가 가능한 권한 높은 원격 엔드포인트라면 방화벽을 클라이언트 IP로 한정하고, 토큰을 유일한 방벽으로 두지 마세요. write 세션 전에는 대상 환경(VM 등)의 스냅샷을 권장합니다.
...
권한 모드는 "편집을 *허용*하는가"의 단일 권위이지만, 편집을 *강제*하지는 않습니다. 에이전트가 순수 질문/분석으로 판단한 요청은 `edit`/`full`에서도 답변 전용으로 남습니다. 모드는 요청이 실제로 요구한 변경을 허용할지만 결정합니다.
...
요청 의도(질문 vs 편집)는 언어 무관하게 분류됩니다. 먼저 빠른 결정론적 휴리스틱이 돌고, 그 오판을 교정하기 위해 LLM 의도 분류기가 기본으로 함께 실행됩니다. 분류기는 스스로는 read-only 쪽으로 좁히기만 하며, 편집 쪽으로 넓히는 것은 권한 모드와 calibration의 통제를 받습니다.

레거시 이름(`default`, `acceptEdits`, `bypassPermissions`)과 Codex 프로파일 id(`:read-only`, `:workspace`, `:danger-full-access`)는 호환을 위해 입력으로는 계속 수용되지만, `/permissions`와 `--help`에는 더 이상 표시되지 않습니다.
...
- `/create-driver-poc <driver-name> [--type objectfilter|minifilter|registryfilter|wfpcallout]`은 x64 전용 C++20 MSVC/WDK driver POC 템플릿을 생성한다. `--type` 생략 시 기존 WDM ping POC를 유지하고, typed template은 objectfilter, minifilter, registryfilter, wfpcallout용 shared IOCTL 계약과 드라이버/테스터 코드를 생성한다. 완료 시 security handoff와 `.kernforge/security/workflow_seed.json`을 남긴다.
