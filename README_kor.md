...
새 세션은 `plan`(기본 읽기 전용)으로 시작하고, `edit`/`full`은 명시적으로 전환합니다. 레거시 이름(`default`, `acceptEdits`, `bypassPermissions`)과 Codex 프로파일 id(`:read-only`, `:workspace`, `:danger-full-access`)도 계속 수용되어 plan/edit/full로 매핑됩니다.
