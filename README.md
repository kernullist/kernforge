...
- `/analyze-project [--path <dir>] [--mode map|trace|impact|surface|security|performance] [goal]` builds reusable project intelligence: an architecture map, deterministic dependency-structure metrics (cycles, coupling, layering, hubs, hotspots, health score), knowledge pack, coverage ledger, evidence packets, performance lens, structural index, vector-ready analysis set, operational docs, and an HTML dashboard.
...
- Request handling v2 runtime with `RequestEnvelope`, `ToolContract`, `TurnRuntimeState`, final-gate decisions, scenario replay fixtures, and optional `request_runtime` migration modes. The runtime keeps review-only and plan-only turns read-only, keeps explicit edit turns tool-driven, preserves mutable verification context through compaction/history rewrite, blocks git mutation unless git was explicitly requested, and stores sanitized shadow divergences only under `.kernforge/request_runtime_shadow`.
...
- Tools report outcomes honestly and fail closed. A chained shell command whose earlier step fails is no longer reported as success and real exit codes are preserved; `grep` results capped at `max_results` include a truncation notice instead of silently appearing complete; `apply_patch` refuses to overwrite an existing file through an `Add File` and preserves insertion position and trailing-newline state; array-form shell arguments can no longer bypass the action boundary; `git_diff`/`git_status` no longer mix CRLF warnings into their output; and namespaced MCP tool names are collision-free so tools from different servers are never silently dropped or mis-routed.
...
- If `compile_commands.json` or other build context exists, Kernforge can prepare a stronger native follow-up; if it does not, Kernforge explains the missing setup before asking whether to continue. Build-only failures are classified and run through a bounded 1–3 self-repair loop with durable blockers.
- Artifacts are stored under `.kernforge/fuzz/<run-id>/` with files such as `report.md`, `harness.cpp`, and `plan.json`. IOCTL surfaces also emit `ioctl_contract.json` and multi-call sequence seeds.
- Native crashes are feasibility-gated (`target_plausible` vs `spurious`); harness-only stacks do not promote as validated findings.
...
- Malformed patches are normalized for common wrapper problems such as BOMs, CRLF/CR line endings, surrounding prose, code fences, and quoted paths. If the same invalid patch signature repeats, the agent is steered to reread the target file and generate a new patch instead of retrying the same text. Hunk matching also tolerates a leading UTF-8 BOM in the target file itself (invisible in read output) and preserves the marker on write, so line-1 hunks on BOM files no longer mismatch forever.
- After stale-context recovery signals such as edit target mismatch or a blocked pre-write review, broad recovery patches are deferred before they touch the workspace. A blocked pre-write proposal is treated as non-committed state: the next edit tool is blocked until the model re-anchors with `read_file`, `grep`, or `git_diff`, then reissues a narrow standalone patch instead of bundling multiple unrelated hunks or submitting a delta against the rejected proposal. `replace_in_file` mismatches now include expected/current context diagnostics (and name ambiguous matches instead of bare "not found"); every mismatch re-requires a reanchor before another context edit. On document-deliverable turns, context edit tools stay disabled after a mismatch until `write_file` succeeds. When only one mismatch budget remains in the turn, the model is steered to `write_file` with the complete final content for whole-file rewrites; a successful edit resets the per-turn mismatch budget, and the mismatch stop report lists which edits already landed this turn instead of claiming nothing was applied.
...
- User-facing output is plain-language and locale-aware. Internal codenames (request class, lifecycle kind, route mode, gate status such as `needs_review`, intervention names) render as human-readable text while the raw values stay only in machine/diagnostic fields. Self-contradictory summaries such as "approved with warnings: 0 warning(s)" now state the real reason, saved review reports lead with the outcome (summary, changed files, findings, next command) and move lifecycle/ledger diagnostics to a trailing Diagnostics section, the MCP review reply leads with a human summary before any JSON, and noisy output is cleaned up (a false-positive secret redaction no longer mangles harmless string literals, PR reports drop Git CRLF warnings, and the live status footer no longer truncates mid-word).
...
- On a single-model route (no independent reviewer configured anywhere), implicit model-backed reviews are skipped automatically with `skipped_single_model_route` and no `Run model review now?` consent prompt appears: the main model reviewing its own edit adds no independent signal, so deterministic checks plus the diff preview are the gate. Explicit `/review` still runs on demand, `review.model_review_consent = always` is the opt-in that keeps implicit self-reviews running, and when a self-review does run its findings stay advisory (they surface as warnings and never hard-block the write).
...
- Request classification follows a read-only-by-default safety rule: ambiguous requests fall toward read-only/least-privilege rather than gaining file or git mutation authority. Run/build/test-only requests are treated as command execution, descriptive questions that merely contain words like update/patch/change stay read-only (verb matching is word-boundary aware so unrelated words such as prefix, suffix, or multistage are not misread as commands), git negations or questions ("do not commit", "what does git push do?") never acquire git permissions, and a stale prior edit no longer lets a later read-only follow-up inherit mutation. A request to analyze code and write it up as a document is classified as document authoring instead of a code-edit lifecycle.
...
The `driver-visibility` preset is intentionally narrow. It captures a lightweight triage snapshot of current driver visibility, verifier state, and related artifacts, not a deep driver load root-cause analysis. For Secure Boot / VBS / HVCI / test-signing / TPM readiness, also run `/investigate start platform-security`.
...
- `/decision` opens the authenticated local Decision Journal dashboard. When code inspection reveals two to four materially valid implementation approaches, KernForge can pause before the first edit, collect an explicit choice plus selected/rejected rationale, and store the result in a private cross-project journal under `~/.kernforge/decision-rationales/`. Browsing, editing, revision history, soft delete/restore, preference rules, rebuild, and JSON export all live inside this one dashboard; there are no `/decision` subcommands. See the detailed [English guide](./docs/decision-journal.md) or [Korean guide](./docs/decision-journal_kor.md).
...
- Computes deterministic dependency-structure metrics (`structure_metrics.json`): package dependency graph, circular-dependency (SCC) detection with break edges, afferent/efferent coupling and instability, architecture layering, PageRank hubs/bottlenecks, orphan files, test topology, git change hotspots and hidden change coupling, and a structure health score; rendered in `STRUCTURE_DIAGRAMS.md` and the map-mode report
- Populates the dependency graph for Go, C/C++, JS/TS, and now Python, C#, and Rust imports so single-language repositories get a real structure map instead of a single collapsed module
...
- Uses native execution only as an optional follow-up. If build context is incomplete, Kernforge explains the gap first instead of silently failing. Build-only failures are classified and run through a bounded 1–3 self-repair loop with durable blockers.
- IOCTL surfaces emit `ioctl_contract.json` and multi-call open/ioctl/close sequence seeds that `/fuzz-campaign` can promote.
...
- `/fuzz-campaign` shows the next recommended campaign action; `/fuzz-campaign run` performs the safe automatic step, including campaign creation, latest `/fuzz-func` attachment, deterministic JSON and IOCTL sequence seed promotion, deduplicated finding lifecycle updates, libFuzzer/llvm-cov/LCOV/JSON coverage report ingestion, sanitizer/verifier/crash-dump artifact capture, coverage gap feedback, artifact graph updates, native result report generation, crash fingerprinting, minimization command capture, evidence recording, `/verify` planner reuse, and tracked feature gate guidance when available.
- Native crashes are feasibility-gated (`target_plausible` vs `spurious`) before finding promotion; harness-only stacks do not become validated findings or source-scan `native-confirmed` feedback. Plausible crashes still merge by fingerprint, source anchor, and suspected invariant.
- `/investigate start platform-security` records best-effort Secure Boot / VBS / HVCI / test-signing / DSE / TPM readiness posture.
